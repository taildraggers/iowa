...
Daily aggregator of Iowa-located taildragger classified listings,
...
[Indiana](https://github.com/taildraggers/indiana) repos' approach, with
the state filter swapped to "IA".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, IA` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Iowa"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/iowa/`
...
```html
<iframe
  src="https://taildraggers.github.io/iowa/"
  title="Taildragger Ads in Iowa"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_IOWA_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
