"""Aggregates Iowa-located listings from every taildraggers.com
...
ones whose location ends in ", IA".
...
PAGE_TITLE = "Taildragger Ads in Iowa"
STATE_ABBR = "IA"
...
_IOWA_LOCATION_RE = re.compile(r",\s*IA\s*$", re.IGNORECASE)


def _is_iowa(location: str) -> bool:
    return bool(_IOWA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        iowa_rows = [row for row in rows if _is_iowa(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(iowa_rows)} in Iowa")
        all_listings.extend(iowa_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Iowa listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Iowa listings: {len(listings)}")
